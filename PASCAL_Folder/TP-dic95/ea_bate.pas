  Program EA_BATE;
                   { Determinazione dei Parametri Orbitali di un Corpo Celeste
                     ...
                     * Da N_Bate a EA_Bate applicato al moto satelliti terrestri
                     * ----------
                     * Cambiano:
                     * ----------
                     * kgauss: da 0.01720209895 a 0.001239446678
                     * i tempi: da giorni in secondi                    
                     * le distanze: da UA a RT=raggio terrestre= 6378.14 km
                     * da coordinate Sole a coordinate Topocentro in RT
                     * da Triv orbita terrestre (Anno siderale)
                     * a Triv orbita sat0 (1 unità= 84.489116756 minuti)
                     * Eliminato il passaggio a coordinate eclittiche, qui sono equatoriali
                     *   }
  Uses  Crt;

    Const
         np=3; npp=6;
         kgauss=0.001239446678; 
         trivsat0=84.489116756;
         rterr=6378.14;
         rad=pi/180;
   Type
  ...
  Procedure LogoDatiLAPL_BATE;
   Begin
     ...
     gotoxy(9,3); writeln('--------------------------------------------------------------');
     textcolor(14);
     gotoxy(15,4); writeln('Nome del File-Dati (senza .DAT) = ');gotoxy(49,4);
     read (w);
     textcolor(10);
...
Skip2: end;
  ...
  Procedure Iterazioner2LAPL_BATE;
   Var
       r2it,V2it,r2ab        : Real;
     Begin
      r2:=2.25; { Ampia stima iniziale del raggio vettore 
                  r2 della 2^ osserv. in unità canoniche, RT }
  repeat
      u2:=1/(r2*r2*r2);
...
q2:= SQR(V2it/r2it) - u2;
 ...
 { Vettori Posizione e Velocita' in Coord. Equatoriali}
    x:=x2; y:=y2; z:=z2; r:=r2;
   Vx:=Vx2; Vy:=Vy2; Vz:=Vz2; V:= V2;
 ...
 gotoxy(23,5);writeln('Semiasse magg. a = ',a*rterr:12:5,' km');
 gotoxy(23,6);writeln('Eccentricita''  e = ',e:12:8);
           if a>0 then
             begin
 gotoxy(23,7);writeln('Periodo Rivol. P = ',Triv*trivsat0:11:5,' minuti');
             end;
 gotoxy(23,8); writeln('Pass.Perigeo  Tp = ',Tper/60:12:5,' minuti da t2');
 gotoxy(23,9); writeln('Dist.Perigeo   q = ',a*(1-e)*rterr:12:5,' km');
 writeln;
 gotoxy(23,11);writeln('Inclinazione   i = ',Incl/rad:12:8,'ø');
 gotoxy(23,12);writeln('Long-Nodo-Asc  ê = ',Nodo/rad:12:8,'ø');
 gotoxy(23,13);writeln('Argom-Perigeo  ì = ',Om/rad:12:8,'ø');
 gotoxy(23,14);writeln('Parametro     ek = ',ek:12:8);
 gotoxy(23,15);writeln('      Cos(omega) = ',(ni*ei+nj*ej)/(n*e):12:8);
