...
C
C
C      subroutine DPLEPH( TDB2, NTARG, NCENT, PV)
C
...
C
C
CC     entry CONST(NAMS,VALS,SS,NCON)
C
...
C      write(6,*)'Requested output units are :'
      if(AU_KM)then
        if(IAU_AU)then
          ...
          write(6,'(a3,i3.3,a16)')' DE',NUMDE,' au for distance'
          if(DAY_SEC)then
            write(6,'(a3,i3.3,a20)')' DE',NUMDE,' au/day for velocity'
          else
            write(6,'(a3,i3.3,a20)')' DE',NUMDE,' au/sec for velocity'
          endif
        endif
      else
