...
5     FINAL_ = 59555     ! 2021/12/07   reported in MJD IERS FINAL_ data (for update) 
      ...
      WRITE (*,*) " ************************************************************************************ "       
      WRITE (*,*) " *         ---------   SOFA_EQUI_SOLST   ----------                                 * "
      WRITE (*,*) " * THIS PROGRAM COMPUTES THE INSTANT OF EQUINOXES AND SOLSTICES                     * "
      WRITE (*,*) " *             FOR PERIODS SUCCESSIVE to YEAR 1962                                  * "
      WRITE (*,*) " *   BEING USED Database JPL DE430t AND SOFA SOFTWARE                               * "
      WRITE (*,*) " *  (S.O.F.A. stands for Standards of Fundamentals Astronomy)                       * "
      WRITE (*,*) " *  Compiling commands (OS Windows/Latex)                                           * "
      WRITE (*,*) " *  gfortran sofa_equi_solst.f90 2020.for jpl_430.f sky9.for -o sofa_equi_solst.exe * "
      WRITE (*,*) " *  gfortran sofa_equi_solst.f90 2020.for jpl_430.f sky9.for -o sofa_equi_solst     * "
      WRITE (*,*) " ************************************************************************************ " 
      WRITE (*,*) "       Date and Times  UTC"    
      WRITE (*,*) "   =============================="
      WRITE (*,*)
      WRITE (*,*) " Input : Year  " 
      READ *, year    
     ...
     WRITE(*,*)"  ====================================================================================================="
10   FORMAT(2X,"-- Spring Equinox ",4x,I4,"/",I2,"/",I2,2x,I2,":",I2,":",F4.1,x,"UTC /",2x,"JD",x,F16.7," TDB",2x, &
                  "Long ",F11.7," degrees") 
  
      CALL equinox( year,FINAL_,EOP_final,L2,M2,IY,MO,ID,IH,IM,SEC,TDB,GELOAPD)
     WRITE(*,20)IY,MO,ID,IH,IM,SEC,TDB,GELOAPD
     WRITE(*,*)"  ====================================================================================================="
20   FORMAT(2X,"-- Summer Solstice",4x,I4,"/",I2,"/",I2,2x,I2,":",I2,":",F4.1,x,"UTC /",2x,"JD",x,F16.7," TDB",2x, &
                  "Long ",F11.7," degrees")

      CALL equinox( year,FINAL_,EOP_final,L3,M3,IY,MO,ID,IH,IM,SEC,TDB,GELOAPD)
     WRITE(*,30)IY,MO,ID,IH,IM,SEC,TDB,GELOAPD
     WRITE(*,*)"  ====================================================================================================="
30   FORMAT(2X,"-- Autumn Equinox ",4x,I4,"/",I2,"/",I2,2x,I2,":",I2,":",F4.1,x,"UTC /",2x,"JD",x,F16.7," TDB",2x, &
                  "Long ",F11.7," degrees") 
 
      CALL equinox( year,FINAL_,EOP_final,L4,M4,IY,MO,ID,IH,IM,SEC,TDB,GELOAPD)
     WRITE(*,40)IY,MO,ID,IH,IM,SEC,TDB,GELOAPD
     WRITE(*,*)"  ====================================================================================================="
40   FORMAT(2X,"-- Winter Solstice",4x,I4,"/",I2,"/",I2,2x,I2,":",I2,":",F4.1,x,"UTC /",2x,"JD",x,F16.7," TDB",2x, &
                  "Long ",F11.7," degrees") 
     WRITE (*,*)
  
      WRITE(*,*)
      WRITE(*,*)" ***************************************************" 
      WRITE(*,*)" Press 'Y' to continue or 'N' to exit. "
      WRITE(*,*)" ***************************************************" 
      READ *, NEW
      IF(NEW == "y")NEW ="Y"
      IF(NEW == "n")NEW ="N" 
...
50    CONTINUE
  end program sofa_equi_solst

! ==================== Ancillary subroutine for Convergence ======================
...
! Radians to degrees
      ...
      tol= 1D-08  ! computing tolerance di calcolo   
      passo= 1D0  ! initial step (1 day)
...
! ============== Dichotomy Method to reach Convergence ============== 

    IF (GELOAPD < 1D0 .AND. MO == 3) then 
         GELOAPD = GELOAPD +360D0 
    END IF 
    IF (GELOAPD > LG) then
         ET = ET - passo
         passo= abs(LG-GELOAPD)/2.0D0
    else
         ET = ET + passo
