...
!
!     Compiling command: gfortran -w planet430F.f90 2121.for jpl_430.f sky9.for -o 2020.exe   
!
!  - - - - - - - - - - -
      IMPLICIT NONE
...
!  Ratio between solar and sidereal time
      ...
      INTEGER IY1, IM1, ID1,IY2, IM2, ID2, IY3, IM3, ID3,IHMSF7(4),IHMSF8(4),NR,MS,PT,LN,height
      INTEGER IHMSF10(4),IHMSF11(4),IHMSF12(4),IHMSF13(4),IHMSF14(4),IHMSF15(4),N1,N2,N3,numday,num,N
      integer,dimension(8) :: VALUES
      DOUBLE PRECISION SEC,XP,YP,DUT,TT,TCB1,TCB2,T,AU,PS,DEL,HA,TDB,Ltime,TDB1,TDB2,TDB0,OBL,s_s,DXD
      ...
      DOUBLE PRECISION latit,UJD,MJD,ANGLE,PANGLE,PDIAM,AZ,ZD,MAGN,AUM,PVV(2),LAST,fract_year
      ...
      DOUBLE PRECISION HE0(3,2),SUNB(3,2),EPV(3,2),GPB(3),HV(3),POS(3),VEL(3),XYZ,V1(3),RA66,RA,RA0
      ...
      DOUBLE PRECISION BODY_H1,GD1,HD1,XET1,PV1,HE01,THD1,E_P(3,2),D_sun,DISTKm,OMEGA,OME0,OBL6,GELO_D,GELA_D
      ...
      DIMENSION body(17),radius(10),dayweek(7),XYZ(3)
      ...
      CHARACTER(len=26):: body,planet,LDUT,LDET,EQ_label,Slod,zone_
      CHARACTER(len=1):: sign,aS,lS,bS,update,NEW,str,AS1,AS2,AS3     
      CHARACTER(len=95):: equinox,equinox2,equinox3,commentR,commentS,commentT,commentU,labelL,site
      CHARACTER(len=45):: LRS,LSE,LTR,TWL
      CHARACTER(len=7):: APE 
      ...
      WRITE (*,*)"  *     New SOFA routines added into 2121.for , JPL DE430t database,       *" 
      WRITE (*,*)"  *     IERS FINAL_ data (IAU2000), and EOP C01 1900-now modified          *"           
      WRITE (*,*)"  *                                                                        *"
      WRITE (*,*)"  *      --------------------planet430F.f90 -------- (Mar.12,2021) --------*"
      WRITE (*,*)"  *                                                                        *"
      ...
      WRITE (*,*)"  *     gfortran planet430F.f90 sky9.for 2121.for jpl_430.f                *"
      ...
      WRITE (*,*)" ****  We recommend that you update at least every 30 days, checking ****  "
...
2     continue    
      FINAL_ = 59625     ! 2022/02/15   reported in MJD IERS FINAL_ data (for update) 

      EOP_final = 59223  ! 2021/12/07 jump from IERS EOP14 C04 to IERS FINAL_ data (IAU2000).       
...
!     DUT = -0.072073685D0               ! Da usare per prove SOFA
      DUT = DUT                          !  sec.
      TUT = TIME + DUT/86400D0           !  in fraction of day          
      LOD = LOD
      XP = XPO * AS2R                    ! arcsec---> radians
      YP = YPO * AS2R   
!  XP = 0.0349282D0 * AS2R               ! Da usare per prove SOFA
!  YP = 0.4833163D0 * AS2R               ! Da usare per prove SOFA
      
      DX00 = DX0/1000D0 * AS2R           ! mas---> radians 
      DY00 = DY0/1000D0 * AS2R 
      
!       DX00 = 0.1725D0 * AS2R/1000D0    ! Da usare per prove SOFA
!       DY00 = -0.2650D0 * AS2R/1000D0   ! Da usare per prove SOFA
...
!--------- Greenwich mean sidereal time (IAU 2000). 
      ...
      GAST = iau_ANP ( iau_GMST00 ( DJMJD0 + DATE_, TUT, DJMJD0, TT ) &
       + iau_EE00 ( DJMJD0, TT, EPSA, DPSI ))

      LAST = GAST + LONG                            ! Local apparent sideral time (IAU 2000)

!-------------------------------------------------------------------------------------------------------      
      EO = ERA - GAST                               ! EO0  Equation of the origin.
      EO0= iau_EORS(RNPB,S)     
!---------------------------------------------------------------------------------------------------   
      EQEQ = iau_EE00A ( TT1, TT2 )                 !Equation of equinox (EQEQ  radians )
...
!--------- Compute  -TDB0- JD Time to 0.0 h TDB the day (for routine TRANSIT)
     ...
     CALL TRANSIT(NTARG,TDB0,GAST0,AUDAY,DELTAT,PL,SD,LAT,LONG,DX00,DY00,RIS,SET,TRS,  &
            AZM1,AZM2,HAL,As,LRS,LSE,LTR,NR,MS,PT)  
...
! ......... Call for direction cosines to spherical coordinates CIRS (RA66 , DE6)
      ...
      IF (mode == 1) then
         RA6 = RA66 - (0.05205D0 * AS2R)    ! 52mas = Diff. RA origin
...
!------Search  civil,nautical,astronomical TWILIGHT   
      ...
      IF(NTARG == 11)THEN 
       ...
       IF(N1 == 1 .or. N2 == 1 .or. N3 == 1) THEN               ! Twilight parameter
          TWL = "---- 00:00:00.0 =   CONTIUOUS TWILIGHT --- "
...
!---------- Convert Geocentric Equatorial (GMB) to Geocentric Ecliptic Coordinate 
      ...
      IF ( GELO < 0D0) THEN
       GELO = GELO + 2D0 * PI
      END IF
!    print*,"GELO_,GELO",GELO_*DR2D,GELO * DR2D,HLON*DR2D
...
5     FORMAT (1x,"Longitude",1x,"deg",F10.5,1x,a,3x,"Latitude",1x,"deg",F9.5,1x,a,3x,"Height",1x,i4,1x,"meter") 
      WRITE(*,*)
...
19    FORMAT('     Earth Rotation Angle ( ERA )    =',f19.13,' degs =',1X,I3,2(":",I2.2),".",I6.6," h,m,s")
      ...
      WRITE(*,*)
      WRITE(*,*)" ------------ Use IERS STANDARD EOP DATA (iau_2000 EOP 14 C04) and iau_2000 A   -------------"
      WRITE(*,*)"        interpolate and corrected for diurnal gravitational and tidal oceanic effect.        " 
      WRITE(*,*)
      WRITE (*,20) XPO,LDUT
...
25    FORMAT("     Nutation in obliquity ",3x,F10.6,1x,"arcsec.")  
      ...
      WRITE (*,*)"==================================================================================================="
      WRITE (*,*)
      WRITE (*,*)" Body : ",planet," JPL planetary ephemeris DE430 "      
      WRITE (*,48)"=================================================================================================="
      WRITE(*,*)
      ...
      WRITE (*,*) "Geocenter mean J2000  equatorial ( GCRS ) and  ...........................ecliptic coordinate   " 
      ...
      WRITE (*,63)
      END IF        
      CALL iau_A2TF ( 5 ,RA10 ,sign , IHMSF )
      CALL iau_A2AF ( 4 ,DE10 ,sign , IDMSF )
      IF ( NTARG == 10 ) THEN
      ...
      IF ( NTARG == 10 ) THEN
        WRITE ( *,61 ) IHMSF ,sign,IDMSF,DIST3 * AUM,Distmoon
        ELSE IF( NTARG /= 10) THEN
      ...
      WRITE (*,48)"==================================================================================================" 
      WRITE (*,*)equinox2 
      WRITE (*,32) "             R.A                     Dec.             Azimuth     (*)Elevation   (*)Hour Angle        "                                      
      WRITE (*,60)                         
      ...
      WRITE(*,*)
       ...
       WRITE(*,*) 
       CALL iau_A2AF ( 4 ,GELO_ ,sign , IDMSF )
        WRITE (*,330)sign,IDMSF,GELO_*DR2D  
        WRITE(*,*)
       CALL iau_A2AF ( 4 ,GELA_ ,sign , IDMSF )
        WRITE (*,340)sign,IDMSF,GELA_ *DR2D        
      END IF 
      WRITE (*,48)"=====================================================================================================" 
      WRITE(*,*)
      WRITE (*,*) "        ILL. %       MAGN.        Angular Diameter        Phase Angle (o)     Sun's Elong.           "
...
300   FORMAT(" Lunar librations: dphi/dt, theta/dt, dpsi/dt (rad/day)",x,f12.10,2x,f12.10,2x,f12.10)      
 ...
 WRITE(*,*)"  Physical parameter   "
           IF (NTARG == 11) THEN 
 WRITE(*,*)"------------------------------------------------------------------------------------------------------------"      
 WRITE(*,*)"  BODY   | Equat.  |         |  Bulk  |  Sidereal   |  Galactic   | Orbital | Spettral |Equatorial| Escape |"
 WRITE(*,*)"         | Radius  | Mass Kg | Density| Rotat.Period| Orbit Period| Velocity|   Class  | Gravity  |Velocity|"
 WRITE(*,*)"         |  (Km)   | x10^24  | g cm^-3|  (day)      |    (year)   |  (Km/s) |          | (m s^-2) | (Km/s) |"           
 WRITE(*,*)"------------------------------------------------------------------------------------------------------------"
 WRITE(*,*)"SUN        1391000   1.9891     1.408    27.275        2.25-2.5       217       G2V        274      617.54 |" 
 WRITE(*,*)"                     *10^30     *10^3                   *10^8                                              |" 
 WRITE(*,*)"------------------------------------------------------------------------------------------------------------"
           END IF
            IF (NTARG == 10) THEN 
 WRITE(*,*)"-----------------------------------------------------------------------------------------------------------------" 
 WRITE(*,*)"  BODY   | Equat.  |         |  Bulk  |   Orbital   |   Synodic   |Eccentri|Ecliptic | Surface | Escape | Geom. |"
 WRITE(*,*)"         | Radius  | Mass Kg | Density|   Period    |   Period    | city   | Inclin. | Gravity |Velocity|Albedo |"
 WRITE(*,*)"         |  (Km)   | x10^24  | g cm^-3|   (day)     |   (day)     | (degs) | (degs)  | (m s^-2)| (Km/s) |       |" 
 WRITE(*,*)"-----------------------------------------------------------------------------------------------------------------"
 WRITE(*,*)"MOON       1738.1     7.342    3.344     27.321661     29.530589    0.0549    5.145     1.62      2.38    0.136 |"
 WRITE(*,*)"-----------------------------------------------------------------------------------------------------------------"
           END IF

           IF (NTARG < 10) THEN
 WRITE(*,*)"-----------------------------------------------------------------------------------------------------------------"      
 WRITE(*,*)"  BODY   | Equat.  |         |  Bulk  |  Sidereal   |   Sideral   | Orbital |Geometric|Equatorial| Escape | num.|"
 WRITE(*,*)"         | Radius  | Mass Kg | Density| Rotat.Period| Orbit Period| Velocity| Albedo  | Gravity  |Velocity|Moons|"
 WRITE(*,*)"         |  (Km)   | x10^22  | g cm^-3|  (day)      |    (year)   |  (Km/s) |         | (m s^-2) | (Km/s) |     |"           
 WRITE(*,*)"-----------------------------------------------------------------------------------------------------------------"
           END IF
           IF (NTARG == 1) THEN
 WRITE(*,*)"MERCURY    2440.53  0.330014   5.4291    58.6462      0.2408467     -0.60     0.106      3.70      4.25     0   |"
 WRITE(*,*)"            ±0.04  ±0.000021  ±0.0007                               ±0.10                                       |"
 WRITE(*,*)"----------------------------------------------------------------------------------------------------------------"
           ELSE IF (NTARG == 2) THEN
 WRITE(*,*)"VENUS      6051.8   4.86747    5.243    -243.018      0.61519726    -4.47     0.65       8.87     10.36     0   |"
 WRITE(*,*)"            ±1.0   ±0.00023   ±0.003                                ±0.07                                       |"
 WRITE(*,*)"-----------------------------------------------------------------------------------------------------------------"
           ELSE IF (NTARG == 3) THEN
 WRITE(*,*)"EARTH    6378.1366   5.97237   5.5136  0.99726968     1.0000174     -3.86     0.367      9.80     11.19     1   |"
 WRITE(*,*)"         ±0.0001    ±0.00028  ±0.0003                                                                           |"
 WRITE(*,*)"-----------------------------------------------------------------------------------------------------------------"
           ELSE IF (NTARG == 4) THEN
 WRITE(*,*)"MARS     3396.19    0.641712   3.9341  1.02595676     1.8808476     -1.52     0.150       3.71     5.03     2   |"
 WRITE(*,*)"         ±0.1       ±0.00003  ±0.0007                                                                           |"
 WRITE(*,*)"-----------------------------------------------------------------------------------------------------------------"
           ELSE IF (NTARG == 5) THEN
 WRITE(*,*)"JUPITER  71492     1898.187    1.3262   0.41354      11.862615      -9.40     0.52       24.79    60.20    79   |"
 WRITE(*,*)"          ±4         ±0.088   ±0.0003                                                                           |"
 WRITE(*,*)"-----------------------------------------------------------------------------------------------------------------"
           ELSE IF (NTARG == 6) THEN
 WRITE(*,*)"SATURN   60268      568.3174   0.6871   0.44401      29.447498      -8.88     0.47       10.44    36.09    82   |"
 WRITE(*,*)"          ±4         ±0.0128  ±0.0002                                                                           |"
 WRITE(*,*)"-----------------------------------------------------------------------------------------------------------------"
           ELSE IF (NTARG == 7) THEN
 WRITE(*,*)"URANUS   25559       86.8127   1.270   -0.71833      84.016846      -7.19     0.51        8.87    21.38    27   |"
 WRITE(*,*)"          ±4         ±0.0040  ±0.001                                                                            |"
 WRITE(*,*)"-----------------------------------------------------------------------------------------------------------------"
           ELSE IF (NTARG == 8) THEN
 WRITE(*,*)"NEPTUNE  24764      102.4126   1.638    0.67125     164.79132       -6.87     0.41       11.15    23.56    14   |"
 WRITE(*,*)"          ±15        ±0.0048  ±0.004                                                                            |"
 WRITE(*,*)"-----------------------------------------------------------------------------------------------------------------"
           ELSE IF (NTARG == 9) THEN
 WRITE(*,*)"PLUTO    1188.3     0.013030   1.89     -6.3872     247.92065       -1.0      0.3         0.62     1.21     5   |"
 WRITE(*,*)"          ±1.6     ±0.000027  ±0.06                                                                             |"
 WRITE(*,*)"================================================================================================================="
           END IF                            
 WRITE (*,*)
      ...
      WRITE (*,*)"       respectively (- 0 d. 50 ') and (-0 d .34 '; lunar semid. and parallax)"
      WRITE (*,*)
...
41    FORMAT (a98)
42    FORMAT ( 6X," h ",I3,2(":",I2.2),".",I1.1,x,a,11X," h ",I3,2(":",I2.2),".",I2.2,x,a,9X," h ",I3,2(":",I2.2),".",I1.1,x,a) 
43    FORMAT ( 11X,F5.1,"d",20X,F4.1,"d",a,20X,F5.1,"d")
39    FORMAT (25x,A45) 
239   FORMAT (25x,A45)
45    FORMAT (25x,A45)
46    FORMAT (25x,A45) 
47    FORMAT (5x,"THE ",x,a8,"NOTES :")
48    FORMAT (A100)
49    FORMAT (A84) 
...
56    FORMAT (9x,F6.2,5x,F6.2,9x,F8.3," arcmin.",10x,F6.2,12x,f5.1,1x,5a) 
58    FORMAT (7x,I3,2x,I2,2x,I2,".",I5.5,6x,a,I3,2x,I2,2x,I2,".",I4.4,4x,F11.3,4x,F11.6,7x,F11.6) 
...
61    FORMAT (7x,I3,2x,I2,2x,I2,".",I5.5,6x,a,I3,2x,I2,2x,I2,".",I4.4,4x,F11.3,7x,F11.6)
62    FORMAT (9x,"h , m ,  s",14x,"o , ' , ''",13x," Km ",12x,"degs",12x,"degs")
63    FORMAT (9x,"h , m ,  s",14x,"o , ' , ''",13x," AU ",14x,"degs",12x,"degs")
...
310   FORMAT (5x,"Apparent Ecliptic Longit.mean the date",2x,a,I3,"d",x,I2,"'",x,I2,"''",".",I4.4,4x,"degs",x,f11.7 )
320   FORMAT (5x,"Apparent Ecliptic Latit. mean the date",2x,a,I3,"d",x,I2,"'",x,I2,"''",".",I4.4,4x,"degs",x,f11.7 )
330   FORMAT (5x,"Ecliptic Longit.mean the date         ",2x,a,I3,"d",x,I2,"'",x,I2,"''",".",I4.4,4x,"degs",x,f11.7 )
340   FORMAT (5x,"Ecliptic Latit. mean the date         ",2x,a,I3,"d",x,I2,"'",x,I2,"''",".",I4.4,4x,"degs",x,f11.7 )
...
1700  CALL iau_CAL2JD ( IY, MO, ID, DJM0, DATE_, J )              ! input TT (IY,MO,ID)
      TDB = DATE_ + TIME                                          ! TDB in MJD

1750  continue                            
     
!----------Call the interpolation routine for date above JD 2437665.5 (1962/01/01) 
!          The value of POLAR MOTION  and OFFSET CELESTIAL POLE are interpolate in
!          step of 0.05 year.(database IERS EOP C01 1900-2000)
!          There will therefore be small deviations from the real values.
!          Which can give errors of about 1-2 mas in RA and DEC
...
!-----Greenwich apparent sidereal time (IAU 2006).
      ...
      LAST = GAST + LONG                            ! Local Apparent Sideral Time
...
!------Update a PVH-vector, discarding the velocity component.
      ...
      CALL iau_PVUP (DT,PVE,EB)                  ! EB Barycentric  position Earth vector
      DO  I=1,3
        BVE(I) = PVE(I,1) * DT + PVE(I,2)        ! BVE Barycentric velocity Earth vector
      END DO                                     
      CALL iau_PPP (EB,GP,OB)                    ! OB = EB + GP (barycentric position of observer)
      CALL iau_PPP (BVE,GV,OV)                   ! OV = BVE + GV (barycentric velocity of observer)
      CALL iau_PVUP (DT,PVB,BP)                  ! BP Body's Barycentric position 
...
!-----------Form  topocentric  position RA0,DE0 ---------------------------------------
       
      CALL L_TIME(ET,NTARG,OB,BVE,HMOD,POS_T,POS_H,DIST,HD0,XET,PV,HE0,THD,EPV)
      POS_T = POS_T        
      DIST4 = DIST
      CALL  iau_C2S ( POS_T, RA0, DE0 ) ! ! RA0, DE0: Topocentric Astrometric coordinate   ( POS_T, RA8, DE8 )       
      RA0 = iau_ANP(RA0)                  ! range angle (0,2!pi)
...
!  calculating "IUA 2000 Equinox based"   
 
       CALL   iau_RXP   ( RNPB, u3, u4 )
       equinox = "Apparent geoc. posit. true equinox and ecliptic of the date.( X-axis  EOP correct by IAU 76/80)" 
...
!      TIME62 = 22645.5D0                ! Tempo in JD tra 1900/01/01 e 1962/01/01 
      CALL   iau_C2S   ( u4, RA66, DE6 )
         ...
         RA6 = RA66 - (0.05650D0 * AS2R)    ! 52mas = Diff. RA origin
         RA6= iau_ANP(RA6)                  ! range angle (0,2!pi)
         DE6 = DE6 
 
      DIST3 = DIST1                              ! Distance 
 
!---------- Form apparent topocentric coordinate RA7,DE7 using "IUA 2000 Equinox based")

       CALL   iau_RXP  ( RNPB, u5, u6 )
       equinox2 = "Apparent Topocentric position  ' no refraction'.(IAU 76/80 EQUINOX BASED)"


! ......... Call for direction cosines to spherical coordinates

5000    CALL   iau_C2S   ( u6, RA7, DE7 )
        RA7 = RA7 - (0.05650D0 * AS2R)             ! 50mas = Diff. RA origin 
         RA7= iau_ANP(RA7)                       ! range angle (0,2!pi)
         RA7H = RA7 * DR2D / 15D0                ! RA7 in hour
         DE7D = DE7 * DR2D
!--------------------------------------------------------------------------------------------
...
!        and compute atmosferic refraction.
      ...
      equinox3 = "Apparent Topocentric position with refraction.(IAU 76/80 EQUINOX BASED)"
...
115   FORMAT ( 1x,"Longitude",1x,"deg",F10.5,1x,a,3x,"Latitude",1x,"deg",F9.5,1x,a,3x,"Height",1x,i4,1x,"meter")       
      WRITE(*,*)
      WRITE(*,116) Dday,IYdate
116   FORMAT ( 1x,"Day of week",1x,a10,3x,"Year and fraction of a year " ,f10.5)
      WRITE(*,*)
...
11    FORMAT ( 1X,A,I5,2("/",I2.2),1X,I3,2(":",I2.2),".",I6.6,"   MJD",F18.11 )
...
1117  FORMAT(x,"UT1-TAI value (1900-1962) ",f11.7," sec")
      ...
      WRITE(*,*)
      WRITE(*,118) GST*DR2D
118   FORMAT('     Greenwich App.Sideral Time (GAST) =',f19.13,' deg', " (IAU 1982/1994 )")
      WRITE(*,1118) GAST*DR2D
1118   FORMAT('     Greenwich App.Sideral Time (GAST) =',f19.13,' deg', " (IAU 2006 )")
      WRITE(*,119) ERA*DR2D
119   FORMAT('     Earth Rotation Angle ( ERA )      =',f19.13,' deg'," (aproximate prior the year 1973 )")
      CALL iau_A2TF ( 6, LAST, SIGN, IHMSF )           
      WRITE(*,126) LAST*DR2D,IHMSF
126   FORMAT('     Local App.Sideral Time (LAST)     =',f19.13,' deg =',1X,I3,2(":",I2.2),".",I6.4," h,m,s", " (IAU 2006 )")

      WRITE(*,*)
      WRITE(*,*)" ============= Use IERS EOP C01 data(1900-1962) ==============="
      WRITE(*,*)
      WRITE(*,*)
      WRITE (*,136) XPO 
...
125   FORMAT (a17,a24 )
      ...
      CALL iau_A2AF ( 4 ,DE0 ,sign , IDMSF )
      IF ( NTARG == 10 ) THEN
        WRITE ( *,154 ) IHMSF ,sign,IDMSF,DIST4 * AUM,Mtime1,deldot1
        ELSE IF( NTARG /= 10) THEN
        WRITE ( *,153 ) IHMSF ,sign,IDMSF,DIST,Ltime1,deldot1
      END IF
      
      WRITE (*,1200)"=================================================================================================="
      WRITE (*,*) "Geocenter mean J2000  equatorial coordinate. ( GCRS ) "
...
!      WRITE (*,147) 
      ...
      WRITE (*,145)LTR 
      WRITE (*,*)
      WRITE (*,*)"   (*)      In the period 1900 - 1962  the value of POLAR MOTION        "
      WRITE (*,*)"       and OFFSET CELESTIAL POLE are interpolate in step of 0.05 year.  " 
      WRITE (*,*)"       (database IERS EOP C01 1900-2000)                                "
      WRITE (*,*)"       There will therefore be small deviations from the real values    "
      WRITE (*,*)"       of apparent coordinate.                                          "
      WRITE (*,*)"       Which can give errors of about 0.5 - 1.0 ms in RA and  0.01 -    "
      WRITE (*,*)"       0.02 arcsec.im DEC.                                              "
      WRITE (*,*)
      WRITE (*,*)"   (*) The option 'IUA 2000 Equinox based'                              "
...
130    FORMAT (7x,I3,2x,I2,2x,I2,".",I5.5,6x,a,I3,2x,I2,2x,I2,".",I4.4,5x,F13.10,"-AU",3x,F10.6," min.")       
131    FORMAT (A98)
132    FORMAT (A98)
135    FORMAT (7x,I3,2x,I2,2x,I2,".",I5.5,6x,a,I3,2x,I2,2x,I2,".",I4.4,5x,F10.6,6x,F11.6,5x,F11.6)
140    FORMAT (7x,I3,2x,I2,2x,I2,".",I5.5,6x,a,I3,2x,I2,2x,I2,".",I4.4,5x,F13.10,5x,F10.6) 
141    FORMAT (a98)
142    FORMAT ( 6X," h ",I3,2(":",I2.2),".",I1.1,x,a,12X," h ",I3,2(":",I2.2),".",I1.1,x,a,9X," h ",I3,2(":",I2.2),".",I1.1,x,a) 
143    FORMAT ( 9X,F5.1,"d",22X,F4.1,"d",a,19X,F5.1,"d")
144    FORMAT (10x," The",1x,A8,"NOTES :") 
145    FORMAT (25x,A40) 
146    FORMAT (25x,A40)
153    FORMAT (7x,I3,2x,I2,2x,I2,".",I5.5,6x,a,I3,2x,I2,2x,I2,".",I4.4,5x,F13.10,"-AU",3x,F10.6," min.",3x,F10.6)
154    FORMAT (7x,I3,2x,I2,2x,I2,".",I5.5,6x,a,I3,2x,I2,2x,I2,".",I4.4,5x,F11.3,"-Km",4x,F9.6," sec.",5x,F10.6)        
160    FORMAT (9x,"h , m ,  s",14x,"o , ' , ''",13x,"degs",12x,"degs",11x,"degs")
161    FORMAT (14x,"R.A",21x,"Dec.",13x,"Azimut",7x,"(*)Elevation",2x,"(*)Hour Angle")
...
1300   FORMAT (7x,I3,2x,I2,2x,I2,".",I5.5,6x,a,I3,2x,I2,2x,I2,".",I4.4,5x,F11.3,"-Km",4x,F9.6," sec.")
1301   FORMAT (7x,I3,2x,I2,2x,I2,".",I5.5,6x,a,I3,2x,I2,2x,I2,".",I4.4,5x,F13.10,"-AU",3x,F10.6)
1302   FORMAT (7x,I3,2x,I2,2x,I2,".",I5.5,6x,a,I3,2x,I2,2x,I2,".",I4.4,5x,F11.3,"-Km",3x,F10.6)        
1303   FORMAT (7x,F6.2,5x,F6.2,6x,"Equat.",F10.3," arcsec.",6x,F6.2,14x,F5.1,1x,5a)  
1304   FORMAT (7x,F6.2,5x,F6.2,9x,F8.3," arcmin.",10x,F6.2,14x,f5.1,1x,5a) 
1306   FORMAT (7x,I3,2x,I2,2x,I2,".",I5.5,6x,a,I3,2x,I2,2x,I2,".",I4.4,3x,F11.3,"-Km",3x,F11.6,5x,F11.6) 
1307   FORMAT (7x,I3,2x,I2,2x,I2,".",I5.5,6x,a,I3,2x,I2,2x,I2,".",I4.4,3x,F13.10,"-AU",3x,F11.6,4x,F11.6) 
