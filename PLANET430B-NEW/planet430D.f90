...
!
!     Compilation: gfortran -w planet430D.f90 1919.for jpl_430.f sky8.for -o a-1919-D.exe
!
!  - - - - - - - - - - -
      IMPLICIT NONE
...
!  Ratio between solar and sidereal time
      ...
      DOUBLE PRECISION DATE_, TIME, DAT,TCB,TCG,POS2(3),DOT,TAI, TUT, UT1, DP80, DE80, MJD0,DJM
      DOUBLE PRECISION RA1,DE1,RA2,DE2,RA3,DE3,RA4,DE4,RA5,DE5,RA6,DE6,RA7,DE7,RA8,DE8,RA9,DE9,ddec,DJM0
      DOUBLE PRECISION RA10,DE10,RMASSE,SUNLONG,COSELON,EDEC,EVET,GLON,GLAT,XEL,DIST,DIST4,ECC,dra,JDtoday         
      ...
      DOUBLE PRECISION latit,UJD,MJD,ANGLE,PANGLE,PDIAM,AZ,height,ZD,MAGN,AUM,Dtime,IMJD,PVV(2),LAST
      ...
      DOUBLE PRECISION iau_ERA00, iau_EE00,iau_EE06A,iau_EORS,iau_EE00A,off_dpsi,off_deps,deldot,C_,OBL0
      ...
      DOUBLE PRECISION TT01,TT02,DE_GMST,EQTMG,UTC0,UTC01,UTC02,RA06,EQTX,BODY_sun(3),R_sun,JH0,MJ,tanAX,AX 
      DOUBLE PRECISION BODY_H1,GD1,HD1,XET1,PV1,HE01,THD1,E_P(3,2),D_sun,DISTKm,LoLu,LoLu_ap,OMEGA,OME0,OBL6
      ...
      DOUBLE PRECISION RVET1,Rdot1,Ltime1,deldot1,Mtime1,HD0,BEV0(3,2),HSVT(3,2),PVBT(3,2),LEA(6)
      DOUBLE PRECISION sine,cose,nsine,ec(3,3),HEPV(3,2),ut1_notid,JH1,DJ4,JHP,JHU,DJ5,DJ6,SSH(3,2),SGV(3,2),tempo
      DOUBLE PRECISION LOsun,Basun,GBsun,XLO,XLA,SUN_ap,SUNapp,GLOsun,LEarth,LSole,PEarth,iau_FAPA03,GELO_,GELA_
      DOUBLE PRECISION SAB(3,2),JUB(3,2),B(8,3),GAST1,ET0(2),uu(3),BCTW,ECTW,BNTW,ENTW,BASTW,EASTW
      ...
      WRITE (*,*)"  **************************************************************************"
      WRITE (*,*)"  *     Last release of Planet, renamed planet430D, to avoid mismatches    *"
      ...
      WRITE (*,*)"  *                                                                        *"
      WRITE (*,*)"  *      --------------------planet430D.f90 -------- (Lug.15,2020) --------*"
      WRITE (*,*)"  *                                                                        *"
      ...
      WRITE (*,*)"  *     gfortran planet430D.f90 sky8.for 1919.for jpl_430.f                *"
      ...
      WRITE (*,*)"  *     ifort -r16 planet430D.f90 1919.for jpl16.for sky8.for              *"
      WRITE (*,*)"  *     which generates the executable file: a.out                         *"
      WRITE (*,*)"  *                                                                        *"
...
210   WRITE (*,*)
      ...
      READ *, IY, MO, ID
       IF(IY < 1900  .OR. IY == 2050 .AND. MO == 2) THEN              
...
!   print*,"DATE,TIME, ZONE, VALUES", DATE,TIMEX, ZONE, VALUES
!   print*,""
!   print*," TODAY, Y/M/D ",VALUES(1),"/",VALUES(2),"/",VALUES(3)
!   print*,"" 
      Ytoday = VALUES(1)
      Mtoday = VALUES(2)
      Dtoday = VALUES(3)
...
!-------------------------------------------------------------------    
      ...
      IF (MJD .GE. 37666) then        ! MJD 41687 = JD 2441687.5 = 1973/01/05.0
       GOTO 178                       ! MJD 37665 = 1962/01/01 ... start database EPO14
      ELSE IF (MJD .LT. 37667)THEN    ! MJD 37665 -- 37671 initial range interpolation  
       GOTO 1640      
      END IF
...
!--------- Greenwich apparent sidereal time (IAU 2000) for 0.0 h UT1.
      ...
      GAST0 = iau_GST00A ( UT_0, 0D0, TT_0, 0D0 )
                     
!--------- Greenwich apparent sidereal time (IAU 2000-2006).

      GAST = iau_GST00A ( UT11, UT12, TT1, TT2 )
...
! ......... Call for direction cosines to spherical coordinates
      ...
      ELSE IF (mode == 2) then
       ...
       CALL iau_ATIO13 ( RA6, DE6, UTC1, UTC2, DUT, LONG, LAT, HM, &      ! AOT = Azimut (CIO Based)
...
!      CALL iau_ATCI13 ( RA3, DE3, 0D0, 0D0, 0D0, 0D0, TT1, TT2, &
!                       RA7, DE7, EO )
!      CALL REPRD ( 'astrometric -> CIRS:', RI, DI )
       ...
       CALL iau_ATIO13 ( RA7, DE7, UTC1, UTC2, DUT,LONG, LAT, HM, XP, &   !RA7,DE7
...
1     FORMAT ( 1X,A,I5,2("/",I2.2),1X,I3,2(":",I2.2),".",I6.6,"   MJD ",F18.11 )
...
!------- Earth  rotation  angle. 
       ERA  =  iau_ERA00  ( DJMJD0+DATE_,TUT)  !(   UTC1,  TUT    ) 
